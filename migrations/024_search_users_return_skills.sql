-- RPC: add skills column (array of speciality strings from ethan_user_speciality).
...
AS $$
  ...
  SELECT
    ...
    (SELECT array_agg(e.speciality ORDER BY e.speciality) FROM ethan_user_speciality e WHERE e.user_id = u.id) AS skills
  FROM "user" u
