...
def clear_all_tables(conn):
    ...
    with conn.cursor() as cur:
        ...
        if case_count + event_count + text_message_count + resource_count == 0:
        ...
        # 1. Delete events (depends on case)
        ...
        # 4. Delete resources (independent)
...
def main():
    ...
    if not force:
        print("\nDatabase URL:", SUPABASE_POSTGRES_URL.split('@')[1].split('/')[0] if '@' in SUPABASE_POSTGRES_URL else "Unknown")
...
def verify_empty(conn):
    ...
    with conn.cursor() as cur:
        tables = ["text_message", "event", '"case"', "resource"]
        all_empty = True
